...
#if 1//defined(_INC_TIME) || defined(_SYS_TIME_H)
...
#if defined(_SHLOBJ_H_) || defined(MINGW_BUILD)
...
LPARAM lParamPointsToParent(HWND Win,LPARAM lPar) {
  ...
  return LPARAM((LOWORD(lPar)+(WinPT.x-ParentPT.x))|((HIWORD(lPar)
    +(WinPT.y-ParentPT.y))<<16));
}
...
void ToolsDeleteAllChildren(HWND ToolTip,HWND Parent) {
  LRESULT c=SendMessage(ToolTip,TTM_GETTOOLCOUNT,0,0);
...
//      Width+=SendMessage(Tree,TVM_GETINDENT,0,0);
    ...
    ChildItem=(HTREEITEM)SendMessage(Tree,TVM_GETNEXTITEM,TVGN_CHILD,
      (LPARAM)Item);
    if(ChildItem) 
...
EasyStr ShortenPath(EasyStr Path,HFONT Font,int MaxWidth) {
  ...
  DrawText(DC,Path,-1,&rc,DT_PATH_ELLIPSIS|DT_LEFT|DT_MODIFYSTRING|DT_NOPREFIX
    |DT_SINGLELINE);
  DeleteDC(DC);
  DeleteObject(Bmp);
  return Path;
