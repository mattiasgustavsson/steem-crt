...
bool macro_play_has_mouse=0,macro_play_has_keys=0,macro_play_has_joys=0;
...
bool macro_mvi_blank(TMacroVblInfo *mvi) {
  if(mvi->xdiff||mvi->ydiff||mvi->keys) 
    return 0;
  for(int i=0;i<8;i++) if(mvi->stick[i]) 
    return 0;
  for(int i=0;i<2;i++) if(mvi->jagpad[i]) 
    return 0;
  return true;
}


void macro_file_options(int GetSet,char *Path,TMacroFileOptions *lpMFO,FILE *f) {
  ...
  if(f)
  {
    ...
    if(GetSet==MACRO_FILE_SET && GetFileLength(f)==0)
    { // Blank file?
      int Dummy=0;
      fwrite(&Version,1,4,f);fwrite(&Dummy,1,4,f);
      fwrite(&Dummy,1,4,f);fwrite(&Dummy,1,4,f);
    }
    fseek(f,0,SEEK_SET);
    fread(&Version,1,4,f);
    if(Version>=2)
  ...
  if(macro_record||(StartCode & MACRO_STARTRECORD))
  {
    ...
    if(macro_record==0)
    {
      ...
      macro_record_store.Resize(macro_record_store.GetSize()
        +Glue.video_freq*MACRO_RECORD_BUF_INC_SECS);
    if(advance)
    {
    ...
    if(macro_record>0)
    {
      ...
      if(f)
      {
        ...
        for(int n=0;n<macro_record;n++) 
          fwrite(&(macro_record_store[n]),1,SizeMVI,f);
        fclose(f);
      }
    }
    macro_record=0;
...
bool macro_play_start() {
  ...
  for(int m=0;m<macro_play_until;m++)
  {
    ...
    fread(lpMVI,1,MIN(sizeof(TMacroVblInfo),(unsigned int)SizeMVI),f);
    if(sizeof(TMacroVblInfo)<SizeMVI)
      fseek(f,SizeMVI-sizeof(TMacroVblInfo),SEEK_CUR);
    bool new_event=0;
    if(lpMVI->keys>0)
    {
      new_event=true;
