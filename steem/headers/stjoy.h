...
class TJoystickConfig, TJoystick
---------------------------------------------------------------------------*/
#pragma once
...
void JoyGetPoses(),JoyPosReset(int);
WORD JoyReadSTEAddress(MEM_ADDRESS addr,bool *pIllegal);
void joy_read_buttons();
BYTE joy_get_pos(int);
bool IsToggled(int);
bool IsJoyActive(int);
DWORD GetJagPadDown(int,DWORD);
bool joy_is_key_used(BYTE);
...
extern "C" BYTE stick[8];

extern void InitJoysticks(int=0),FreeJoysticks();
...
private:
  ...
  static LRESULT CALLBACK DeadZoneWndProc(HWND,UINT,WPARAM,LPARAM);
  static LRESULT CALLBACK GroupBoxWndProc(HWND,UINT,WPARAM,LPARAM);
