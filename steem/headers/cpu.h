...
*/
  ...
  inline void sr_check_z_n_l_for_r0(); //TODO, used by stemdos
  bool IsPriv(WORD op);
  // DATA
...
BYTE check_ipl(); // return ipl level IF above mask in SR, 0 otherwise
extern COUNTER_VAR tvn_latch_time;
extern TIplTiming ipl_timing[256];
extern BYTE ipl_timing_index;

#define CHECK_IPL tvn_latch_time=ACT // macro placed in each opcode function
...
#define SR_VALID_BITMASK 0xa71f // unused now
...
inline void TMC68000::sr_check_z_n_l_for_r0() { //TODO, used by stemdos
  result=r[0];
  SR_CHECK_Z_AND_N_L;
}
...
#define CPU_BUS_IDLE(t)               pInstructionTimeIdle(t) // max is 8
...
extern "C" unsigned getDivu68kCycles( unsigned long dividend, 
  unsigned short divisor);
extern "C" unsigned getDivs68kCycles( signed long dividend, 
...
#define CHECK_STOP_ON_USER_CHANGE  \
            if(stop_on_user_change){ \
              if ((debug_old_sr & SR_SUPER) && (PSWS)==0) stop_on_user_change=2;  \
            }
