...
void TDiskManager::MoveDrag() {
  ...
  {
    Okay=true;
    if(OverID==102&&GetDlgCtrlID(DragLV)==102)
          ...
          if(DragInf->Zip && enable_zip)
          {
            if(DragLV!=DiskView)
             // Dragged from one floppy ListView to another
              DiskInZip=FloppyDisk[GetDlgCtrlID(DragLV)-100].DiskInZip;
            else
            {
              EasyStringList esl;
        ...
        (i==80&&AtHome==0&&DragLV==DiskView))
      {
        ...
        if(RightDrag)
        {
          ...
          if(DragInf->LinkPath.IsEmpty())
          {
            ...
            if(DragInf->Folder==0) 
              AppendMenu(OpMenu,MF_STRING,4010
                ,T("Create M&ultiple Shortcuts Here"));
            if(DestFol!=DisksFol && DragInf->Folder==0)
            {
              AppendMenu(OpMenu,MF_STRING,4012,
                T("Move Disk Here and &Get Contents"));
              AppendMenu(OpMenu,MF_STRING,4011,
          ...
          {
            if(DestFol!=DisksFol) 
              AppendMenu(OpMenu,MF_STRING,4003,T("&Move Shortcut Here"));
            ...
            if(NotSameStr_I(FolDisk,DestFol))
              AppendMenu(OpMenu,MF_STRING,4000,LPSTR(DragInf->Folder
                ?T("Move &Folder Here"):T("Move &Disk Here")));
            AppendMenu(OpMenu,MF_STRING,4001,LPSTR(DragInf->Folder
        ...
        EasyStr SrcFol=LPSTR((MenuTarget==4000||MenuTarget==4001||MenuTarget==
          4011||MenuTarget==4012)?DragInf->Path:DragInf->LinkPath);
        ...
        else if(((MenuTarget==4001&&DragInf->Folder==0)||MenuTarget==4004)&&SrcFol==DestFol)
        {
          ...
          if(MenuTarget==4001) 
            UpdateBPBFiles(DragInf->Path,Path,0);
          if(MenuTarget==4001)
        ...
        else if(MenuTarget==4000||MenuTarget==4001||MenuTarget==4011
          ||MenuTarget==4012|| //Move/Copy Path
        ...
        {  
          ...
          if(DoIt)
          {
            if(MoveOrCopyFile(Moving,From,To,DiskPath,
  ...
  else
  {
    ...
    for(int disk=0;disk<2;disk++)
    {
      if(InDrive[disk])
      {
        ...
        FloppyDisk[disk].DiskName=OldName[disk];
        TDiskManFileInfo *DriveInf=GetItemInf(0,GetDlgItem(Handle,100+disk));
        if(DriveInf) 
          DriveInf->Path=Dest;
      }
    }
